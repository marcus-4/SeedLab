...
BotState currentState = DEMO_START;
...
const float desiredRotVelocity_DEMO_INIT_SEARCH = pi / 10;
const float rotationThreshold_DEMO_TRACKING_ROT = 0.001;  // Radians
const float perpendicularDist_DEMO_TRACKING_ROT = feetToMeters(1);
...
const float desiredCircleVelocity_DEMO_CIRCLING = feetToMeters(1);
...
void loop() {
  ...
  switch (currentState) {
    ...
    case DEMO_APPROACH:
      desiredRot = 0;
      if (currentDist >= (approachDist + botLength - FF_APPROACH)) {
        resetControlLoop();
        transitionNoise();
        resetControlLoop();
  ...
  */
  ...
  if (millis() >= lastSampleTime + desired_Ts) {
    ...
    */
    ...
    Motor_L.setVoltage(V1);
    Motor_R.setVoltage(V2);
